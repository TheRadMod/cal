...
def main():
    ...
    parser.add_argument('-r', '--rate', required= True, type=float, help="Rate of return")
    ...
    if len(total_value_time_combos) <= 0:
    ...
    my_npv = 0
    for value, date in zip(values_list, date_list):
        my_npv += (value * ((1 + args.rate/100) ** ((today - date).days / 365)))
